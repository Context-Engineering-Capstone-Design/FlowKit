...
def client_error(payload: ClientErrorIn, request: Request, db: DbSession, user: OptionalUser):
    ...
    item = ClientErrorLog(trace_id=getattr(request.state, 'trace_id', ''), user_id=user.id if user else None, client_error_type=payload.client_error_type[:80], message=text, page_context=_safe_context(payload.page_context), user_agent=(request.headers.get('user-agent') or '')[:300])

...
FastAPI + PostgreSQL(SQLAlchemy) 기반 백엔드. 설계 근거는 `../../FlowKit-기획/기획 문서/`의
요구사항정의서·BE기능명세서를 따른다.
...
블록을 이어붙여 전체 흐름을 구성한다. 원본 대화가 항상 보존된다(NFR-007).
...
(REQ-021, REQ-041, REQ-042).

**정제 기준 스냅샷** — `block_refine_targets`가 정제 실행 시점의 활성 버전과 본문을 고정한다.
이후 원본이 바뀌어도 정제 기준은 흔들리지 않는다(BE-REFINE-001).

...
- Project 폴더 안 대화를 드래그해 다른 Project 폴더나 `최근 대화`(Project 밖) 영역에 놓으면 옮겨진다. 메인 대화를 옮기면 기존 규칙대로 그 사이드 채팅 트리도 함께 움직인다.
...
`0820_19` QA에서 실제 브라우저로 처음 확인했다. 코드 수정 없이 문서 서술만 정확하게 고쳤다.

- Project 폴더 안 "새 대화" → `최근 대화`(Project 밖)로 드래그해 옮기는 것을 확인했다. `PATCH /api/projects/chats/{chatId}`가 200으로 응답하고 사이드바 목록이 즉시 갱신됐다.
- 위 "구현 결과"의 "선택 상자로 이동"이라는 서술은 실제 구현(드래그 앤 드롭)과 달라 바로잡았다. 선택 상자(드롭다운) UI는 존재하지 않는다.

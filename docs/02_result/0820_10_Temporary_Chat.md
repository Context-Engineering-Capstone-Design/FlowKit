...
> 2026-08-20 후속 변경: 하위 사이드 채팅의 Temporary 강제 규칙은 `0820_16`에서 제거됐다. 어느 깊이에서든 일반 또는 Temporary를 선택할 수 있다.
...
- [x] D3. 탭을 닫으면 임시 대화가 즉시 사라지는지 2026-08-21 QA에서 확인. 새로고침 후 미복원·만료 정리는 미확인(아래 "QA 후속 확인" 참고)
- [x] D4. 첫 자식 선택 UI를 2026-08-21 QA에서 확인(모달이 아니라 "Temporary" 토글 방식). 하위 노드 강제 규칙은 `0820_16`에서 이미 제거됨
...
`0820_19` QA에서 처음 실제 Chrome으로 확인했다(`0820_20` 통합 대화 노드 전환 이후 화면 기준). 코드 변경 없음.

- 사이드 채팅 생성 화면에 "새 사이드 채팅 만들기"·"Temporary" 두 옵션이 있음을 확인했다. "Temporary"는 만들기 전에 켜고 끄는 토글이다(선택 모달이 아님).
- Temporary로 만든 채팅은 탭에 시계 아이콘이 붙고, 좌측 "대화 구조"/대화 목록에는 나타나지 않는다.
- Temporary 탭을 닫으면 탭 목록에서 즉시 사라진다.
- 이 환경은 OpenAI 연결이 안 돼 Temporary Chat에서 실제 답변을 받는 것, 새로고침 후 미복원, 부모 Context 추가/메시지 가져오기/형제 브랜치 버튼의 숨김 여부를 일반 사이드 채팅과 나란히 비교하는 것까지는 확인하지 못했다.
